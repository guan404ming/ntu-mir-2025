...
class TraditionalMLPipeline:
    ...
    def preprocess_data(self, X_train, y_train, X_val, y_val):
        ...
        print(f"Feature scaling completed. Mean: {X_train_scaled.mean():.3f}, Std: {X_train_scaled.std():.3f}")
    ...
    def train_models(self, X_train, y_train):
        ...
        with tqdm(total=len(models_to_train), desc="Training models", unit="model", colour="blue") as pbar:
        ...
        print(f"\nTraining completed!")
        print(f"SVM - Best params: {self.models['SVM'].best_params_}, CV score: {self.models['SVM'].best_score_:.4f}")
        print(f"RF - Best params: {self.models['RandomForest'].best_params_}, CV score: {self.models['RandomForest'].best_score_:.4f}")
        print(f"k-NN - Best params: {self.models['KNN'].best_params_}, CV score: {self.models['KNN'].best_score_:.4f}")
    ...
    def evaluate_models(self, X_val, y_val):
        ...
        with tqdm(total=len(model_names), desc="Evaluating models", unit="model", colour="green") as pbar:
        ...
        print(f"\nEvaluation Results:")
        for name in results:
            print(f"  {name}: Top-1={results[name]['top1_accuracy']:.4f}, Top-3={results[name]['top3_accuracy']:.4f}")
    ...
    def generate_reports(self, X_val, y_val, results, best_model_name):
        ...
        plt.tight_layout()
        plt.savefig(f'results/task1/confusion_matrix_{best_model_name.lower()}.png',
        ...
        bars1 = ax1.bar(model_names, top1_scores, color=['#ff7f0e', '#2ca02c', '#1f77b4'])
        ...
        bars2 = ax2.bar(model_names, top3_scores, color=['#ff7f0e', '#2ca02c', '#1f77b4'])
    ...
    def predict_test_set(self):
        ...
        top3_labels = self.label_encoder.inverse_transform(top3_indices.flatten()).reshape(-1, 3)
